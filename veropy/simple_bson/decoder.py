...
from .etc import I32, I64, U64, TypeSignature, DecodeError
...
def read_name(stream: io.BytesIO):
    ...
    while True:
        ...
        if stream_read == b"":
            raise DecodeError(
                f"Cannot Read Name : Buffer : {buffer} : Bytes Left : {stream.read()}"
            )
...
def decode_element(element_type: int, stream: io.BytesIO):
    try:
        ...
        raise DecodeError(
            f"No decoder for : signature {element_type} :  Bytes Left : {stream.read()}"
        )
...
def decode_int32(stream: io.BytesIO) -> I32:
    return I32(struct.unpack("<i", stream.read(4))[0])


@register((TypeSignature.int64,))
def decode_int64(stream: io.BytesIO) -> I64:
    return I64(struct.unpack("<q", stream.read(8))[0])


@register((TypeSignature.uint64,))
def decode_uint64(stream: io.BytesIO) -> U64:
    return U64(struct.unpack("<Q", stream.read(8))[0])
